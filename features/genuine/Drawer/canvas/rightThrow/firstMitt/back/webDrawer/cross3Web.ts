...
import { verticalParts, verticalParts2 } from './parts/vertical'

const stitchOfCross3Web = (ctx: CanvasRenderingContext2D, color: string) => {
  ctx.lineWidth = 1.5
...
export const cross3Web = (ctx: CanvasRenderingContext2D, state: GenuineState) => {
  webTop2(ctx, state.web.color, state.baseModel) // ウェブ先端側
  webBottom2(ctx, state.web.color, state.baseModel) // ウェブ捕球面側

  stitchOfCross3Web(ctx, state.stitch.color)
  verticalParts2(ctx, state)
  // ウェブ上の結び目
